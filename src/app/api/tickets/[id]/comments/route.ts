...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  if (!(await consumeRateLimit(auth.actor.userId, 'ticket.comment'))) {
    return NextResponse.json({ ok: false, error: 'Limite de comentários atingido. Aguarde alguns minutos.' }, { status: 429 });
  }
  const { id } = await context.params;
  const form = await request.formData();
  const fileValue = form.get('file');
  const file = fileValue instanceof File && fileValue.size > 0 ? fileValue : null;
  const validation = validateCommentInput({ message: String(form.get('message') || ''), hasFile: Boolean(file) });
  ...
  try {
    const result = await addPortalComment({ actor: auth.actor, ticketId: id, message: validation.value.message, file });
