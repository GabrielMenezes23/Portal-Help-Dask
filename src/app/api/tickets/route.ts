...
import {
  assignOpeningResponsible,
  validateOpeningResponsibleSelection,
...
export async function GET(request: Request) {
  ...
  try {
    ...
    return NextResponse.json(
      { ok: false, error: cause instanceof Error ? cause.message : 'Falha ao consultar chamados.' },
      { status: 500 },
    );
  }
}
...
export async function POST(request: Request) {
  ...
  if (!(await consumeRateLimit(auth.actor.userId, 'ticket.create'))) {
    return NextResponse.json(
      { ok: false, error: 'Limite de abertura de chamados atingido. Aguarde alguns minutos.' },
      { status: 429 },
    );
  }
  ...
  let openingResponsible: { optionId: string; label: string };
  try {
    openingResponsible = await validateOpeningResponsibleSelection({
      optionId: String(form.get('openingResponsibleOptionId') || ''),
      label: String(form.get('openingResponsibleName') || ''),
    });
  } catch (cause) {
    return NextResponse.json(
      {
        ...
        errors: {
          openingResponsible:
            cause instanceof Error ? cause.message : 'Selecione o responsável pela abertura.',
        },
      },
      { status: 400 },
    );
  ...
  try {
    ...
    const responsibleResult = await assignOpeningResponsible({
      ...
      label: openingResponsible.label,
    });
    return NextResponse.json(
      {
        ok: true,
        ...result,
        responsibleSyncError: responsibleResult.mondaySyncError,
      },
      { status: 201 },
    );
  } catch (cause) {
    return NextResponse.json(
      { ok: false, error: cause instanceof Error ? cause.message : 'Falha ao criar chamado.' },
      { status: 500 },
    );
  }
}
